...
class LSTMBatchPlayer(Player):
    def __init__(self, pid, actions, alpha, beta, gamma, horizon, hidden_size, num_layers, lr, lr_decay, device, batch_size=1, log_freq=0):
        self.pid = pid
    ...
    def update(self, t, state, get_history):
        if t > 2:
            ...
            if (t-1) % self.batch_size == 0:
                ...
                Q_pred = self.model(seq).to(self.device)
                Q_target = torch.tensor(np.vstack(Q_target_list), dtype=torch.float32).to(self.device)
                loss = self.loss_func(Q_pred, Q_target)

                self.loss_tot += loss.cpu().detach().item()
                self.loss_cnt += time_ed - time_st
